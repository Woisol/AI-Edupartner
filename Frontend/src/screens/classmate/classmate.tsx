		...
		<div className="w-screen h-screen relativ bg-[#FAFAFA]">
			{originText ?
				<>
					...
					<Button variant="contained" color="error" size="large" className="!fixed right-3 bottom-3" onClick={handleClear}><Close />清空</Button>
				</> :
				<>
					<div className={`absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 bg-white rounded-2xl transition-all duration-300 ${dropping ? 'size-[500px]' : 'size-[400px]'}`} onDragOver={handleDragOver} onDragLeave={handleDragLeave} onDrop={handleDrop}>
						{!originText && <h2 className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-3xl text-nowrap font-bold text-center whitespace-pre-wrap pointer-events-none transition-all duration-300 ${dropping ? 'opacity-0' : 'opacity-100'}`}>{`拖拽文件到此\n支持音频、文本、PPT`}</h2>
						}
					</div>
